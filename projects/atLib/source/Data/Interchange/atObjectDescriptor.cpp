...
bool atObjectDescriptor::AsBool(const bool &defaultVal) const { return IsValue() ? atScan::Bool(GetNode().value) : defaultVal; }
int64_t atObjectDescriptor::AsInt(const int64_t &defaultVal) const { return IsValue() ? atScan::Int(GetNode().value) : defaultVal; }
double atObjectDescriptor::AsFloat(const double &defaultVal) const { return IsValue() ? atScan::Float(GetNode().value) : defaultVal; }
atString atObjectDescriptor::AsString(const atString &defaultVal) const { return IsValue() ? GetNode().value : defaultVal; }
