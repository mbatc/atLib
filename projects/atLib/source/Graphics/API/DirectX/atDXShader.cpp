...
static void _BindConstantBuffer(const atPipelineStage &stage, atDXBuffer *pBuffer, int64_t slot)
{
  ...
  ID3D11DeviceContext *pCtx = (ID3D11DeviceContext*)pDX->GetContext();
  ID3D11Buffer *pDxBuf = validBuffer ? (ID3D11Buffer*)pBuffer->NativeResource() : nullptr;
  switch (stage)
  {
...
static void _BindSampler(const atPipelineStage &stage, atDXSampler *pBuffer, int64_t slot)
{
  ...
  ID3D11DeviceContext *pCtx = (ID3D11DeviceContext*)pDX->GetContext();
  ID3D11SamplerState *pDxBuf = validBuffer ? (ID3D11SamplerState*)pBuffer->NativeResource() : nullptr;
  switch (stage)
  {
...
bool atDXShader::BindTexture(const atString &, atTexture *) { atRelAssert("DirectX is only supported on Windows platforms."); return false; }
bool atDXShader::BindSampler(const atString &, atSampler *) { atRelAssert("DirectX is only supported on Windows platforms."); return false; }
